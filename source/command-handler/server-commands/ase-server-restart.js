...
const { createServerRestartAuditEmbed } = require('../../services/utilities/embed-audits/embeds');
      ...
      await Promise.all(services.map(async identifiers => {
        ...
        try {
          ...
          await interaction.followUp({ embeds: [createServerManagementSuccessEmbed(token)] }).then(async () => {
            ...
            await channel.send({ embeds: [createServerRestartAuditEmbed(token, interaction.user.id, 1)] });
          });
